...
export interface mainInterface {
    ...
    registerFile(name: string, size: bigint, mimeType: string, blobHash: string): Promise<string>;
    saveDocument(docId: string, title: string, htmlContent: string): Promise<boolean>;
}
import type { Document as _Document, FileMetadata as _FileMetadata, UserRole as _UserRole, _CaffeineStorageRefillInformation as __CaffeineStorageRefillInformation, _CaffeineStorageRefillResult as __CaffeineStorageRefillResult } from "./declarations/main.did.d.ts";
export class Main implements mainInterface {
    ...
    async createDocument(arg0: string): Promise<string> {
        if (this.processError) {
            try {
    ...
    async registerFile(arg0: string, arg1: bigint, arg2: string, arg3: string): Promise<string> {
        if (this.processError) {
            try {
...
function from_candid_opt_n12(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Document]): Document | null {
    return value.length === 0 ? null : value[0];
}
function from_candid_opt_n13(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_FileMetadata]): FileMetadata | null {
    return value.length === 0 ? null : value[0];
}
